...
func ParseColor(colorStr string) (svg.Color, error) {
	...
	if cStr != colorStr {
		...
		if len(vals) != 3 {
			return svg.InvalidColor, ErrParamMismatch
		}
		var cvals [3]uint8
		for i, v := range vals {
		...
		return svg.ValidColor(cvals[0], cvals[1], cvals[2], 0xFF), nil
	}
	...
	if colorStr[0] == '#' {
		...
		return svg.ValidColor(uint8(r), uint8(g), uint8(b), 0xFF), nil
	}
	return svg.InvalidColor, ErrParamMismatch
}
