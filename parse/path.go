...
func (p *Path) addRect(minX, minY, maxX, maxY, rot float64) {
	...
	m := matrix.Identity.Translate(cx, cy).Rotate(rot).Translate(-cx, -cy)
...
func (p *Path) addRoundRect(minX, minY, maxX, maxY, rx, ry, rot float64) {
	...
	stretch := rx / ry
	midY := minY + h/2
	m := matrix.Identity.Translate(minX+w/2, midY).Rotate(rot).Scale(1, 1/stretch).Translate(-minX-w/2, -minY-h/2)
